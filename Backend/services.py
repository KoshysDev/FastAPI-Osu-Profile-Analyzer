...
async def get_current_user(db: _orm.Session = _fastapi.Depends(get_db), token: str = _fastapi.Depends(oauth2schema)):
    try:
        ...
        raise _fastapi.HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid Email or Password")
...
async def _lead_selector(lead_id, user: _schemas.User, db: _orm.Session):
    lead = db.query(_models.Lead).filter_by(owner_id=user.id).filter(_models.Lead.id == lead_id).first()

    if lead is None:
        raise _fastapi.HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Lead does not exist")

    return lead

async def get_lead(lead_id, user: _schemas.User, db: _orm.Session):
    lead = await _lead_selector(lead_id=lead_id, user=user, db=db)

    return _schemas.Lead.from_orm(lead)

async def delete_lead(lead_id, user: _schemas.User, db: _orm.Session):
    lead = await _lead_selector(lead_id, user, db)

    db.delete(lead)
    db.commit()

async def update_lead(lead_id, lead: _schemas.LeadCreate, user: _schemas.User, db: _orm.Session):
